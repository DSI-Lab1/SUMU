 ...
 * This is the fourth version that uses four upper bounds (PEUO, RSUO, PES, and RSS)
 * and the minsup filtering strategy.
 */


public class AlgoHUOSPM_PES {
	...
	public void runAlgorithm(String input, String output, int minSup, double minUO) throws IOException {
		...
		for (Node node : nodeList) {
			...
			// call the HUOSPM_PES function
			HUOSPM_PES(node, revisedDataBase, minuo, minsup);
	 ...
	 * @throws IOException
	 */
	private void HUOSPM_PES(Node node, Map<Integer, List<List<UItem>>> revisedDataBase, double minuo, int minsup)
			throws IOException {
		...
		// prune early
		boolean remove = node.judge(minsup, minuo);
		if(remove) {
			return;
		}
		...
		if (lists != null) {
			...
			for (Node n : iNode) {
				...
				// call the HUOSPM_PES function
				if (sup >= minsup) {
					HUOSPM_PES(n, revisedDataBase, minuo, minsup);
				}
	...
	public void printStats() {
		System.out.println("===========  HUOSPM_PES ALGORITHM - STATS =========");
	...
	private List<List<Node>> extension(Node node, Map<Integer, List<List<UItem>>> revisedDataBase, double minuo, int minsup) {
		...
		Map<Integer, Integer> ssMap = new HashMap<Integer, Integer>();
		...
		for (Integer key : iMap.keySet()) {
			...
			Integer supValue = iiMap.get(key);
			if (rsuValue < minuo || supValue < minsup) {
		...
		for (Integer key : sMap.keySet()) {
			...
			Integer supValue = ssMap.get(key);
			if (rsuValue < minuo || supValue < minsup) {
