 ...
 * This is the third version that uses two upper bounds (TPUO and TSUO)
 * and the minsup filtering strategy.
 * The priority queue is utilized to calculate these two upper bounds.
 */


public class AlgoHUOSPM_TPUO {
...
//	 	}
		...
		for (Node node : nodeList) {
			...
			// call the HUOSPM_TPUO function
			HUOSPM_TPUO(node, revisedDataBase, minuo, minsup);
	 ...
	 * @throws IOException
	 */
	private void HUOSPM_TPUO(Node node, Map<Integer, List<List<UItem>>> revisedDataBase, double minuo, int minsup)
			throws IOException {
		...
		// prune early
		double tpuo = node.getTPUO(minsup);
		if (tpuo < minuo) {
			return;
		}
		...
		if (lists != null) {
			...
			for (Node n : iNode) {
				...
				// call the HUOSPM_TPUO function
				if (sup >= minsup) {
					HUOSPM_TPUO(n, revisedDataBase, minuo, minsup);
				}
	...
	public void printStats() {
		System.out.println("===========  HUOSPM_TPUO ALGORITHM - STATS =========");
	...
	private List<List<Node>> extension(Node node, Map<Integer, List<List<UItem>>> revisedDataBase, double minuo, int minsup) {
		...
		Map<Integer, PriorityQueue<Double>> iMap = new HashMap<Integer, PriorityQueue<Double>>();
		Map<Integer, PriorityQueue<Double>> sMap = new HashMap<Integer, PriorityQueue<Double>>();
		...
		for (Integer sid : sidSet) {
			...
			for (Integer key : tmp) {
				if (iMap.get(key) == null) {
					PriorityQueue<Double> pm = new PriorityQueue<Double> (minsup, new MyComparator());
					pm.add(sPeuo);
					iMap.put(key, pm);
				} else {
					PriorityQueue<Double> pm = iMap.get(key);
					if(pm.size() == minsup) {
						if(sPeuo > pm.peek()) {
							pm.poll();
							pm.add(sPeuo);
			...
			for (Integer key : tmp) {
				if (sMap.get(key) == null) {
					...
					PriorityQueue<Double> pm = sMap.get(key);
					if(pm.size() == minsup) {
						if(sPeuo > pm.peek()) {
							pm.poll();
							pm.add(sPeuo);
