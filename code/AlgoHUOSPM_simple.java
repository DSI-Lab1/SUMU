 ...
 * This is the simplest version, using only two upper bounds PEUO and RSUO.
 */


public class AlgoHUOSPM_simple {
...
//	 	}
		...
		for (Node node : nodeList) {
			...
			// call the HUOSPM_simple function
			if(sup >= minsup) {
				HUOSPM_simple(node, revisedDataBase, minuo, minsup);
			}		
		}
	 ...
	 * @throws IOException
	 */
	private void HUOSPM_simple(Node node, Map<Integer, List<List<UItem>>> revisedDataBase, double minuo, int minsup)
			throws IOException {
		...
		if (lists != null) {
			...
			for (Node n : iNode) {
				...
				// call the HUOSPM_simple function
				if (sup >= minsup) {
					HUOSPM_simple(n, revisedDataBase, minuo, minsup);
				}
	...
	public void printStats() {
		System.out.println("===========  HUOSPM_simple ALGORITHM - STATS =========");
